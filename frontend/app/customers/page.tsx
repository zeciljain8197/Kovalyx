...
const TIER_COLORS: Record<string, string> = {
  bronze: 'text-kovalyx-bronze border-l-kovalyx-bronze',
  silver: 'text-kovalyx-silver border-l-kovalyx-silver',
  gold: 'text-kovalyx-gold border-l-kovalyx-gold',
}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {tiers.map((tier) => (
          <Card key={tier.key} className={`border-l-4 ${TIER_COLORS[tier.key]}`}>
            ...
            <p className={`mt-1 text-2xl font-bold ${TIER_COLORS[tier.key].split(' ')[0]}`}>{tier.count}</p>
          </Card>
        ))}
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <KpiCard title="Total Customers" value={tierSummary.total_customers} />
        <KpiCard title="Average Total Spent" value={formatCurrency(tierSummary.avg_total_spent)} />
      </div>

      <div>
        <h2 className="mb-3 text-lg font-semibold text-gray-200">Cohort Retention</h2>
