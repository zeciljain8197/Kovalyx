...
function ReturnRateTooltip({ active, payload, label }: { active?: boolean; payload?: { payload: ReturnTrend }[]; label?: string }) {
  ...
  return (
    <div className="rounded-md border border-gray-200 bg-white p-3 text-xs text-gray-700 shadow-lg dark:border-gray-800 dark:bg-gray-900 dark:text-gray-200">
      ...
      <p className="mt-1 text-red-600 dark:text-red-400">Return rate: {formatPct(row.return_rate)}</p>
      <p className="text-gray-500 dark:text-gray-400">Returned orders: {row.returned_orders}</p>
      <p className="text-gray-500 dark:text-gray-400">Total orders: {row.total_orders}</p>
    </div>
  )
}
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <ComposedChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="period_date" tickFormatter={formatPeriodLabel} stroke={theme.axis} fontSize={12} />
