...
function GmvTooltip({ active, payload, label }: { active?: boolean; payload?: { payload: DailySales }[]; label?: string }) {
  ...
  return (
    <div className="rounded-md border border-gray-800 bg-gray-900 p-3 text-xs text-gray-200 shadow-lg">
      ...
      <p className="mt-1 text-kovalyx-gold">GMV: {formatGmv(row.total_gmv)}</p>
      <p className="text-gray-400">Orders: {row.total_orders}</p>
    </div>
  )
}
    ...
    <ResponsiveContainer width="100%" height={300}>
      <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
        ...
        <XAxis
          ...
          fontSize={12}
        />
        <YAxis tickFormatter={formatGmv} stroke="#6b7280" fontSize={12} />
